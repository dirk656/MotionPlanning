...
def get_robot_collision_bodies_at_zero(links, joints, base_pos):
    link_transforms = {}
    T_base = np.eye(4)
    T_base[:3, 3] = base_pos
    
    # Identify the base link(s)
    # They are the ones that don't appear as a 'child' in any joint
    child_links = {j['child'] for j in joints}
    for link_name in links:
        if link_name not in child_links:
            link_transforms[link_name] = T_base
            
    for joint in joints:
        parent_T = link_transforms.get(joint['parent'], T_base)
        T_joint_origin = make_transform(joint['xyz'], joint['rpy'])
        link_transforms[joint['child']] = parent_T @ T_joint_origin

    bodies = []
    for link_name, link_info in links.items():
        T_link = link_transforms.get(link_name, T_base)
...
if __name__ == "__main__":
    ...
    if urdf_path:
        ...
        robot_collision_bodies = get_robot_collision_bodies_at_zero(links, joints, base_pos=base_pos)
